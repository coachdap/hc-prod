	...
	private function getListSettings()
	{
		...
		if (empty($lists)) {
			$lists = [
				[
					'value' => '',
					'label' => __('No lists available, please check api key?','ninja-forms-hubspot')
				]
			];
		}
		...
		$this->settings[$prefix . 'newsletter_list_fields'] = array(
			...
			'label' => __('List Field Mapping','ninja-forms-hubspot'),
			'type' => 'fieldset',
			'group' => 'primary',
			'settings' => array()
		);
		...
		$this->settings[$prefix . 'newsletter_list_groups'] = array(
			...
			'label' => __('Preselected Interest Groups','ninja-forms-hubspot'),
			'type' => 'fieldset',
			'group' => 'primary',
			'settings' => array()
		);
