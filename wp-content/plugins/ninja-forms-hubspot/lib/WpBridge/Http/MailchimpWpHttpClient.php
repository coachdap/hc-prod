...
namespace NFHubspot\EmailCRM\WpBridge\Http;
 ...
 */
class MailchimpWpHttpClient implements MailchimpHttpClientInterface
{
	/**
	 * @inheritdoc
	 */
	public function handleRequest($method, $uri = '', $options = [], $parameters = [], $returnAssoc = false)
	{
		...
		if (is_wp_error($response)) {
			throw new MailchimpAPIException($response->get_error_message(), $response->get_error_code());
		} else {
			...
			throw new MailchimpAPIException(substr($status, 0, 1), $status);
		}
	}
}
