...
add_action( 'wp_ajax_ampforwp_get_licence_activate_update', 'amp_ninja_form_get_extension_licence_activate_update' );

function amp_ninja_form_get_extension_licence_activate_update(){
        ...
        if( isset($selectedOption['amp-license']) && "" != $selectedOption['amp-license']){
            // data to send in our API request
                ...
                if ( ! empty( $message ) ) {
                    ...
                    $limit = amp_ninja_form_set_extension_plugin_limit( true, $license_data, $ampforwp_license_activate);
...
function amp_ninja_form_extension_deactivate_license() {
    if(!wp_verify_nonce( $_REQUEST['verify_nonce'], 'verify_extension' ) ) {
        echo json_encode(array("status"=>300,"message"=>'Request not valid'));
        die;
    }
...
add_action( 'wp_ajax_ampforwp_deactivate_license', 'amp_ninja_form_extension_deactivate_license' );


function amp_ninja_form_set_extension_plugin_limit( $force=false, $license_data='', $data) {
